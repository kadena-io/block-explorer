...
transactionsPage
  ...
  -> BlockHeader
  -> m ()
transactionsPage netId cid bp bh = do
    let txs = _blockPayloadWithOutputs_transactionsWithOutputs bp
        bhash = _blockHeader_hash bh
    ...
    elClass "table" "ui celled table" $ do
      ...
      el "tbody" $ do
        forM_ txs $ \(t, tout) -> do
          let route = addNetRoute netId (unChainId cid) $ Chain_BlockHash :/ (hashB64U bhash, Block_Transaction :/ (T.pack $ show $ _transaction_hash t))
          el "tr" $ el "td" $ case _pactCommand_payload $ _transaction_cmd t of
            ExecPayload exec ->  routeLink route $ text $ _exec_code exec
            ContPayload cont ->  routeLink route $ text $ _cont_pactId cont
...
transactionPage
  ...
  -> BlockPayloadWithOutputs
  -> m ()
transactionPage netId cid bp = el "p" $ text "transaction page under construction"
-- TODO: get transaction information from the tx hash passed into the url to determine what to display next
-- TODOL create abstract widget that takes tx hash and displays what used to be displayed in the accordion dropdown (see line 75-139)
