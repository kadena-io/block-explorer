...
transactionPage netId cid bp = do
  dynTxHash :: Dynamic t Text <- askRoute
  let txs = M.fromList $ map (\(t, tout) -> ((T.pack $ show $ _transaction_hash t), (t, tout))) $  _blockPayloadWithOutputs_transactionsWithOutputs bp
  _ <- switchHold never <=< dyn $ ffor dynTxHash $ \txHashTxt -> do
    let mTout = M.lookup txHashTxt txs
    case mTout of
      Nothing -> do
        el "p" $ text "Transaction Details not found."
        return never
      Just (t, tout) -> do
